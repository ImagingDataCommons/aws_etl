...
duplicate_view="uuid_duplicates"
...
if __name__=="__main__":
  query_string = "SELECT * FROM "+athena_database_name+"."+duplicate_view
  #params=["false"]
  df_duplicates=fetch_query_to_dataframe(athena_client, s3_client, s3_output_location,query_string)
  dest_df=df_duplicates.query("file_found==False")
  src_df=df_duplicates.query("file_found==True")

  for index, row in src_df.iterrows():
    uuid = row['uuid']
    url = row['pub_aws_url']
    src_series=url.split('/')[3]
    src_loc=src_series+'/'+uuid+'.dcm'
    dest_row=dest_df.query("uuid=='"+uuid+"'").iloc[0]
    dest_series=dest_row['pub_aws_url'].split('/')[3]
