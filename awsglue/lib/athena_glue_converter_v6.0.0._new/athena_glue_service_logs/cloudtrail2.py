...
class CloudTrailRawCatalog2(BaseCatalogManager):
...
class CloudTrailConvertedCatalog2(BaseCatalogManager):
    ...
    def conversion_actions(self, dynamic_frame):
        ...
        raw_fields = ['requestParameters', 'responseElements', 'userIdentity', 'additionalEventData','tlsDetails']
        json_mappings = [(name, 'struct', "json_%s" % name.lower(), 'string') for name in raw_fields]
        json_mappings.append(('resources','array','json_resources','string'))
        raw_fields.append('resources')
        ...
        mapped_dyf = mapped_dyf.drop_fields(paths=['addendum', 'serviceEventDetails'])
