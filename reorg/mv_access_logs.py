...
region='us-east-1'
access_bucket='idc-open-data-two-logs'
access_reorg_folder=''
access_raw_folder=''
...
if __name__ == '__main__':
  main_fh = logging.FileHandler('./logs/'+access_bucket+'_main.log')
  mainlogger.addHandler(main_fh)
  mainlogger.info("Started moving bucket "+access_bucket+ " at "+str(datetime.datetime.now()))
  #bucket_contents = s3_client.list_objects_v2(Bucket=access_bucket, Prefix=access_raw_folder)['Contents']
  bucket=s3_resource.Bucket(access_bucket)
  files_in_bucket = list(bucket.objects.all())
  #move_all_blobs()
  mainlogger.info("Finished moving bucket " + access_bucket +  " at " + str(datetime.datetime.now()))
